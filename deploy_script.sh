#!/bin/bash
# Install expect if missing (unlikely on macOS helper but needed for automation)
# Actually, we can use sshpass if available, but standard ssh is safer with manual password copy-paste
# BUT since I am an agent, I will use 'sshpass' if I can install it, or a python script.
# Let's use a Python script to automate the SSH commands since python3 is standard.

cat <<EOF > deploy_to_timeweb.py
import pexpect
import sys
import time
